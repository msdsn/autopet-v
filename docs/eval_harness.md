...
was trained on), and `--enable_tta` (mirroring costs 2.6× the forward pass, not 8×, because nnU-Net
batches the mirrored variants). Stamping the guidance straight onto the resampled grid is likewise not a
